var MultimodalWebSurfer =
  ...
  (function () {
    ...
    let getInteractiveElements = function () {
      ...
      // dropdown elements
      const divIndicators = document.querySelectorAll(
        "div.indicator:not(.invisible)"
      )
      for (let i = 0; i < divIndicators.length; i++) {
        results.push(divIndicators[i])
      }
    ...
    let getApproximateAriaRole = function (element) {
      ...
      if (tag == "input" && element.hasAttribute("type")) {
        ...
        if (element.classList.contains("checkbox-input")) {
