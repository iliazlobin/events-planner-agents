...
from autogen_agentchat.base import Response, TerminatedException, TerminationCondition
from autogen_agentchat.messages import AgentEvent, ChatMessage, MultiModalMessage, StopMessage, TextMessage, ToolCallExecutionEvent
...
class MultimodalWebSurfer(BaseChatAgent, Component[MultimodalWebSurferConfig]):
    ...
    # termination_condition = FunctionCallTermination(function_name="complete") | FunctionCallTermination(function_name="error")
    ...
    def __init__(
        ...
        if model_client.model_info["function_calling"] is False:
            raise ValueError("The model does not support function calling. MultimodalWebSurfer requires a model that supports function calling.")
    ...
    async def _lazy_init(
        ...
        if sys.platform == "win32":
            ...
            if hasattr(asyncio, "WindowsProactorEventLoopPolicy") and not isinstance(current_policy, asyncio.WindowsProactorEventLoopPolicy):
                warnings.warn(
        ...
        if self._context is None:
            if self.browser_data_dir is None:
                if self.connect_over_cdp:
                    browser = await self._playwright.chromium.connect_over_cdp(self.connect_over_cdp)
    ...
    async def _generate_reply(self, cancellation_token: CancellationToken) -> UserContent:
        ...
        if len(other_targets) > 0:
            ...
            other_targets_str = "Additional valid interaction targets include (but are not limited to):\n" + "\n".join(other_targets) + "\n\n"
        else:
            other_targets_str = ""

        state_description = "Your " + await self._get_state_description()
        tool_names = "\n".join([t["name"] for t in tools])
        all_tool_names = "\n".join([t["name"] for t in tools + self.termination_tools])
        ...
        # Make the request
        all_tools = tools + self.termination_tools
        response = await self._model_client.create(
            history, tools=all_tools, extra_create_args={"tool_choice": "auto"}, cancellation_token=cancellation_token
        ...
        if isinstance(message, str):
            ...
            # Take an action
            return await self._execute_tool(message, rects, all_tool_names, cancellation_token=cancellation_token)
        else:
    ...
    async def _execute_tool(
        ...
        if name == "visit_url":
            ...
            if target_name:
                action_description = f"I clicked the target: '{target_name}'."
            else:
        ...
        elif name == "expand_dropdown_menu":
            target_id = str(args.get("target_id"))
            target_name = self._target_name(target_id, rects)
            if target_name:
                action_description = f"I clicked the dropdown menu: '{target_name}'."
            else:
                action_description = "I clicked the control."
            new_page_tentative = await self._playwright_controller.click_id(self._page, target_id)
            if new_page_tentative is not None:
            ...
            print(f"Completion status: {status}, reasoning: {reasoning}")
            self.termination_condition.set()
            # cancellation_token.cancel()
            return "COMPLETED"
        elif name == "error":
            message = str(args.get("message"))
            reasoning = str(args.get("reasoning"))
            print(f"Error message: {message}, reasoning: {reasoning}")
            self.termination_condition.set()
            # cancellation_token.cancel()
            return "ERROR"
        ...
        if self._last_download is not None and self.downloads_folder is not None:
            ...
            await self._page.goto("data:text/html;base64," + base64.b64encode(page_body.encode("utf-8")).decode("utf-8"))
            await self._page.wait_for_load_state()
    ...
    def _format_target_list(self, ids: List[str], rects: Dict[str, InteractiveRegion]) -> List[str]:
        ...
        for r in list(set(ids)):
            if r in rects:
                ...
                actions = ['"click", "hover"']
                if rects[r]["role"] in ["dropdown"]:
                    actions = ['"expand_dropdown_menu"']
    ...
    def _from_config(cls, config: MultimodalWebSurferConfig) -> Self:
        return cls(
            ...
            connect_over_cdp=config.connect_over_cdp,
        )
